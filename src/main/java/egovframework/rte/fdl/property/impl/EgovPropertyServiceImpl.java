...
/**
 * 전자정부 프레임워크의 egovframework.rte.fdl.property.impl.EgovPropertyServiceImpl 클래스가 버그가 있어서 이것을 수정한 클래스
 * 관련 내용은 https://www.egovframe.go.kr/uss/olh/qna/QnaInqireCoUpdt.do?qaId=QA_00000000000019135&menu=5&submenu=3 를 참고
 * (원래 전자정부 프레임워크 가이드라인에서는 egovframework.rte 패키지 이하로는 어떤 클래스도 만들면 안되지만 위의 링크에서 재정의해서 사용하는것으로 허가를 받았음)
 * 
 * @author Terry Chang
 *
 */
		...
		try {
			...
			if (properties != null) {
				...
				while (it.hasNext()) {
					...
					// 이 부분이 기존 클래스에서는 버그여서 아래와 같이 수정했다
